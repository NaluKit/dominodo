...
public class TasksRepository {
  ...
  private void addInitialData() {
    ...
    task1.getTags()
         .addAll(Arrays.asList("Bug",
                               "Milestone 1.0"));
    task1.getAttachments()
         .addAll(Arrays.asList("File 1",
                               "File 2"));
    ...
    task2.getTags()
         .addAll(Arrays.asList("Enhancements",
                               "Router",
                               "Milestone 1.0"));
    task2.getAttachments()
         .addAll(Arrays.asList("File 1",
                               "File 2",
                               "File 3"));
    ...
    task5.getTags()
         .addAll(Arrays.asList("Question",
                               "Invalid",
                               "Milestone 3.0"));
    task5.getAttachments()
         .addAll(Arrays.asList("File 1",
                               "File 2",
                               "File 3",
                               "File 4"));
    ...
    task6.getTags()
         .addAll(Arrays.asList("Anime",
                               "Ghibli"));
    task6.getAttachments()
         .addAll(Arrays.asList("File 1"));
    ...
    return tasks.stream()
                .filter(task -> {
                  ...
                  JsDate taskDate = new JsDate((double) task.getDueDate()
                                                            .getTime());
                  return todayDate.getYear() == taskDate.getYear() && todayDate.getMonth() == taskDate.getMonth() && todayDate.getDate() == taskDate.getDate() && task.isActive();
  ...
  public List<Task> listNextWeekTasks() {
    return tasks.stream()
                .filter(task -> {
                  ...
                  return diff > 0 && diff <= 604800000 && task.isActive();
